...
def apply_rotary_emb_qwen(
    ...
    preprocessed_cos_sin: Tuple[torch.Tensor, torch.Tensor] = None,  # 新增参数
    ...
    if use_real:
        ...
        if not ROPE_FUSE:       #-----------------   ljf  --------------------
            ...
            # ========== 核心优化：复用预处理后的cos/sin，跳过重复的expand/reshape ==========
            if preprocessed_cos_sin is not None:
                cos, sin = preprocessed_cos_sin
                # # ========== 新增：维度校验日志 ==========
                # print(f"[RoPE维度校验] x.shape: {x.shape}, cos.shape: {cos.shape}, sin.shape: {sin.shape}")
                # # 确保cos/sin的seqlen与x的seqlen一致
                # assert cos.shape[1] == x.shape[1], f"cos seqlen {cos.shape[1]} != x seqlen {x.shape[1]}"
                # assert sin.shape[1] == x.shape[1], f"sin seqlen {sin.shape[1]} != x seqlen {x.shape[1]}"
                # # 确保cos/sin的最后一维与x的最后一维一致
                # assert cos.shape[-1] == x.shape[-1], f"cos dim {cos.shape[-1]} != x dim {x.shape[-1]}"
                # assert sin.shape[-1] == x.shape[-1], f"sin dim {sin.shape[-1]} != x dim {x.shape[-1]}"
            else:
                # 兼容未缓存的情况（保留原逻辑）
                cos = freqs_cis.real
                sin = freqs_cis.imag
...
def compute_text_seq_len_from_mask(
    ...
    per_sample_len = torch.where(
        ...
        torch.as_tensor(text_seq_len, device=active_positions.device)  # 指定设备为active_positions的设备（NPU）     # ljf
    )
...
class QwenDoubleStreamAttnProcessor2_0:
    ...
    def __init__(self):
        ...
        # ========== 新增：RoPE cos/sin预处理缓存 ==========
        self._rope_cos_sin_cache = {}  # key: (freqs_cis_hash, device), value: (cos, sin)
        self._rope_cache_lock = threading.Lock()

        self.algo = int(os.getenv('ALGO', 0))  
        head_dim = 128
        self.scale=(head_dim ** -0.5)

    def _preprocess_rope_cos_sin(self, freqs_cis, device):
        # ========== 核心修复：替换复数张量的sum+item()，避免ComplexFloat报错 ==========
        # 用形状+步长+设备作为唯一key（无需计算sum，避免复数操作）
        freqs_shape = tuple(freqs_cis.shape)
        freqs_stride = tuple(freqs_cis.stride())
        cache_key = (freqs_shape, freqs_stride, device)  # 替代原hash逻辑
        # local_rank = int(os.getenv("LOCAL_RANK", 0))

        with self._rope_cache_lock:
            if cache_key not in self._rope_cos_sin_cache:
                # 仅首次执行cos/sin的预处理（原逻辑不变，仅去掉hash）
                # print(f"[Rank {local_rank}] RoPE cos/sin缓存未命中，首次预处理！cache_key: {cache_key}")
                cos = freqs_cis.real  # 提取实部，转为实数张量（避免复数操作）
                sin = freqs_cis.imag  # 提取虚部，转为实数张量
                seqlen = cos.shape[0]
                
                # 优化后的形状变换（减少stridedSlice）
                # cos = cos.reshape(1, seqlen, 1, -1).repeat(1, 1, 1, 2)
                # sin = sin.reshape(1, seqlen, 1, -1).repeat(1, 1, 1, 2)
                cos = cos.unsqueeze(0).unsqueeze(2).unsqueeze(-1).expand(-1, -1, -1, -1, 2).reshape(1, seqlen, 1, -1)
                sin = sin.unsqueeze(0).unsqueeze(2).unsqueeze(-1).expand(-1, -1, -1, -1, 2).reshape(1, seqlen, 1, -1)
            
                
                # 转设备+统一dtype（避免后续cast）
                cos = cos.to(device, non_blocking=True, dtype=torch.bfloat16)
                sin = sin.to(device, non_blocking=True, dtype=torch.bfloat16)
                self._rope_cos_sin_cache[cache_key] = (cos.detach(), sin.detach())
            else:
                # print(f"[Rank {local_rank}] RoPE cos/sin缓存命中，直接复用！")
                cos, sin = self._rope_cos_sin_cache[cache_key]
        return cos, sin
    ...
    def __call__(
        ...
        if image_rotary_emb is not None:
            ...
            # ========== 优化2：调用apply_rotary_emb_qwen时传入预处理后的cos/sin（修改原函数） ==========
            # 先修改apply_rotary_emb_qwen函数，支持直接传入预处理后的cos/sin（避免重复计算）
            img_query = apply_rotary_emb_qwen(
                img_query, img_freqs, use_real=False, 
                preprocessed_cos_sin=(img_cos, img_sin) if ROPE_FUSE else None  # 新增参数
            )
            img_key = apply_rotary_emb_qwen(
                img_key, img_freqs, use_real=False,
                preprocessed_cos_sin=(img_cos, img_sin) if ROPE_FUSE else None
            )

            txt_query = apply_rotary_emb_qwen(
                txt_query, txt_freqs, use_real=False,
                preprocessed_cos_sin=(txt_cos, txt_sin) if ROPE_FUSE else None
            )
        ...
        if self.algo == 0:
            ...
            joint_hidden_states = attention_forward(
                joint_query, 
                joint_key, 
                joint_value,
                opt_mode="manual", 
