...
class GroupCoordinator:
    ...
    def all_gather(
            self, input_: torch.Tensor, dim: int = 0, separate_tensors: bool = False, async_op: bool = False
        ...
        # All-gather.
        if async_op:
            current_input_size = input_size.copy()  # 复制列表
            current_world_size = world_size
            current_dim = dim
            current_separate_tensors = separate_tensors
            comm = torch.distributed.all_gather_into_tensor(  # ljf 报错
                output_tensor, input_, group=self.device_group, async_op=async_op
            )

            def getter():
                comm.wait()
                nonlocal output_tensor  # 声明为非局部变量

                if current_dim != 0:
                    # 使用捕获的变量，而不是外部变量
                    temp_size = current_input_size
                    temp_size[0] //= current_world_size
                    output_tensor = output_tensor.reshape([current_world_size] + temp_size)
                    output_tensor = output_tensor.movedim(0, current_dim)

                if current_separate_tensors:
                    tensor_list = [
                    ...
                    current_input_size[current_dim] = current_input_size[current_dim] * world_size
                    # Reshape
                    output_tensor = output_tensor.reshape(current_input_size)
                    return output_tensor

            return getter
        else:
            torch.distributed.all_gather_into_tensor(  # ljf 报错
