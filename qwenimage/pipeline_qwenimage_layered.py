                    ...
                    if get_classifier_free_guidance_rank() == 0:
                        ...
                        with self.transformer.cache_context("uncond"):
                            noise_pred = self.transformer(
                                ...
                                additional_t_cond=is_rgb,
                                return_dict=False,
                                use_cache=UNCOND_CACHE,
                                if_cond=False,
                            )[0]
                            noise_pred = noise_pred[:, : latents.size(1)]
                    noise_pred = noise_pred.contiguous()  # 新增：确保张量连续
                    noise_pred_cond, noise_pred_uncond = get_cfg_group().all_gather(noise_pred, separate_tensors=True)
